...
namespace BlazorSozluk.Api.Application.Features.Commands.User
{
    public class LoginUserCommandHandler : IRequestHandler<LoginUserCommand, LoginUserViewModel>
    {
        ...
        {
            var dbUser = await this.userRepository.GetSingleAsync(x => x.EmailAddress == request.EmailAddress);
            ...
            var password = PasswordEncryptor.Encyrpt(request.Password);

            if (dbUser.Password != password)
            {
                throw new DatabaseValidationException("Username or password  is wrong");
            }
